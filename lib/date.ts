...
// 与えられた日付（ローカル/ISO）に対するJSTのその日の開始(00:00)をUTCのDateとして返す
export function jstStartOfUtc(anyDate: Date | string): Date {
  const d = typeof anyDate === 'string' ? new Date(anyDate) : anyDate
  const iso = formatInTimeZone(d, JST, 'yyyy-MM-dd')
  // isoはJSTでの日付文字列。これをDateにするとUTCのその時刻(=JST 00:00相当)になる
  return new Date(iso)
}

export function jstTodayStartUtc(): Date {
  return jstStartOfUtc(new Date())
}
